...
/*
	...
	- Simulation Settings:
        ...
        - Statistics Output:
                ...
                - Trace output file: Hyscale.out is located in the /statistics folder
...
//
	...
	int num_bridge = num_host;	// number of bridges per rack
	int num_agg = 8;
	int total_hosts = num_hosts*pow(num_agg,k+1);
	char filename [] = "statistics/HyScale.xml";// filename for Flow Monitor xml output file
...
//	
	std::cout << "Number of Levels in Topology (k) =  "<< k<<"\n";
...
//
  	...
  	Simulator::Stop (Seconds(100.0));
        std::string animFile = "statistics/fat3-netanim.xml";
        AnimationInterface anim(animFile);

	
  	AsciiTraceHelper ascii;
  	csma.EnableAsciiAll (ascii.CreateFileStream ("statistics/fat-tree.tr"));
	int pos=10;
	for (i=0; i<num_group;i++){  	
	  for (j=0; j<num_core;j++) {
		Ptr<Node> n = core[i].Get(j);
		anim.UpdateNodeSize((*n).GetId(), 3, 3);
		anim.UpdateNodeColor((*n).GetId(),255.0,0.0,0.0);
		anim.SetConstantPosition (n, pos,10,0);
		pos+=5;
	  }
	}
	...
	for (i=0; i<num_pod;i++){  	
	  for (j=0; j<num_agg;j++) {
		Ptr<Node> n = agg[i].Get(j);		
		anim.UpdateNodeSize((*n).GetId(), 3, 3);
		anim.UpdateNodeColor((*n).GetId(),0.0,255.0,0.0);
		anim.SetConstantPosition (n, pos, 20,0);
		pos+=5;
	}
	}
	...
	pos=5;
  	for (i=0; i<num_pod;i++){  	
	  for (j=0; j<num_bridge;j++) {
		Ptr<Node> n = edge[i].Get(j);
		anim.UpdateNodeSize((*n).GetId(), 3, 3);
		anim.UpdateNodeColor((*n).GetId(),0.0,0.0,255.0);
		anim.SetConstantPosition (n, pos, 30,0);
		pos+=5;
	}
	}
	pos=5;
  	for (i=0; i<num_pod;i++){  	
	  for (j=0; j<num_bridge;j++) {
		Ptr<Node> n = bridge[i].Get(j);
		anim.UpdateNodeSize((*n).GetId(), 3, 3);
		anim.UpdateNodeColor((*n).GetId(),0.0,0.0,255.0);
		anim.SetConstantPosition (n, pos, 35,0);
		pos+=5;
	}
	}
	...
	pos=0;
  	for (i=0; i<k;i++){
		for (j=0;j<num_bridge;j++){  	
			for (int l=0;l<num_host;l++){  	
			Ptr<Node> n = host[i][j].Get(l);
			anim.UpdateNodeSize((*n).GetId(), 3, 3);
			anim.UpdateNodeColor((*n).GetId(),255.0,0.0,255.0);
			anim.SetConstantPosition (n, pos, 50,0);
			pos+=5;
	}
	}
	} 
